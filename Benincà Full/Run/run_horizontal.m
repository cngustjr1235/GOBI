...
t_eq = (t_ori(1):time_interval:t_ori(end)).';
...
%% run with horizontal window 

fixed_target = 4;

% Preprocess data - horizontal cut
...
y_total = [cyc, pro, rot, cycdot];
t_total = t_eq;

[y_cut, t_cut, class_pair_list, class_count_list] = cut_horizontal(y_total, t_total, {[], [1], [1], []});

%% normalize
for idx_data = 1:length(y_cut)
    y_data = y_cut{idx_data};
    for idx_component = 1:length(y_data(1,:))
        y_comp = y_data(:, idx_component);
        y_comp = y_comp - min(y_comp);
        y_comp = y_comp / max(y_comp);
        y_data(:,idx_component) = y_comp;
    end
    y_cut{idx_data} = y_data;
end

%% Compute 1D RDS
thres_noise_1D = 0;
[S_total_list_1D, R_total_list_1D, pair_list_1D] = RDS_1D_new(y_cut, t_cut, fixed_target, num_component, type_self, thres_noise_1D);
...
%% Compute 2D RDS
thres_noise_2D = 0;
[S_total_list_2D, R_total_list_2D, pair_list_2D] = RDS_2D_new(y_cut, t_cut, fixed_target, num_component, type_self, thres_noise_2D);

%% Compute 3D RDS
thres_noise_3D = 0;
[S_total_list_3D, R_total_list_3D, pair_list_3D] = RDS_3D_new(y_cut, t_cut, fixed_target, num_component, type_self, thres_noise_3D);

%% Compute 2D TRS
if thres_R == 0
    thres_R = 0.05;
end
S_total_list_2D = S_total_list_2D(:,:,1);
R_total_list_2D = R_total_list_2D(:,:,1);
...
% Surrogate Test
[boot_candidate_list_2D, boot_type_list_2D, surrogate_list_2D] = surrogate_test_2D(y_cut(1), t_cut{1}, delta_candidate_list_2D, delta_type_list_2D, delta_list_2D, p_delta, p_surrogate, type_self, thres_noise_2D);
