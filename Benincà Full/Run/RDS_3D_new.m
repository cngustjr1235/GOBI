function [S_total_list, R_total_list, pair_list] = RDS_3D_new(y_total, t_total, target, num_component, type_self, thres_noise)
...
cause_pair_list = nchoosek(component_list, 3);
pair_list = [];
...
parfor data = 1:num_data
    ...
    for pair = 1:length(pair_list(:,1))
        ...
        cause3_index = pair_list(pair,3); % index for cause3
        target_index = pair_list(pair,4); % index for target
        
        % compute regulation-detection function
        if type_self == -1
            [score_list, t_1, t_2] = RDS_ns_dim3(y(:,cause1_index), y(:,cause2_index), y(:,cause3_index), y(:,target_index), t, nan);
        elseif type_self == 1
            [score_list, t_1, t_2] = RDS_ps_dim3(y(:,cause1_index), y(:,cause2_index), y(:,cause3_index), y(:,target_index), t, nan);
        else
            [score_list, t_1, t_2] = RDS_dim3(y(:,cause1_index), y(:,cause2_index), y(:,cause3_index), y(:,target_index), t, nan);
        end
