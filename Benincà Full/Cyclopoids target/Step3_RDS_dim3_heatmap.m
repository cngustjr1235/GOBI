...
st1 = component_list_dim3(component_index,1); % index for cause1
st2 = component_list_dim3(component_index,2); % index for cause2
st3 = component_list_dim3(component_index,3); % index for cause2
ed = component_list_dim3(component_index,4);  % index for target

% y_target(:, st1) = (y_target(:,st1) - min(y_target(:,st1))) / max(y_target(:,st1));
% y_target(:, st2) = (y_target(:,st2) - min(y_target(:,st2))) / max(y_target(:,st2));
% y_target(:, ed) = (y_target(:,ed) - min(y_target(:,ed))) / max(y_target(:,ed));

% compute regulation-detection function
if type_self == -1
    [score_list, t_1, t_2] = RDS_ns_dim3(y_target(:,st1), y_target(:,st2), y_target(:,st3), y_target(:,ed), t_target, time_interval);
elseif type_self == 1
    [score_list, t_1, t_2] = RDS_ps_dim3(y_target(:,st1), y_target(:,st2), y_target(:,st3), y_target(:,ed), t_target, time_interval);
else
