...
% data_ori = [data_ori.CalanoidCopepods, data_ori.FilamentousDiatoms, data_ori.Picophytoplankton, data_ori.Nanophytoplankton, data_ori.Bacteria];
% labels = ["CalanoidCopepods", "FilamentousDiatoms", "Picophytoplankton", "Nanophytoplankton", "Bacteria"];
data_ori = [data_ori.Cyclopoids, data_ori.Protozoa, data_ori.Rotifers, data_ori.CalanoidCopepods];
labels = ["Cyclopoids", "Protozoa", "Rotifers", "CalanoidCopepods"];
...
% cyc_raw = data_suppress(:,1);
% cyc_raw_min = min(cyc_raw(cyc_raw > 0));
% 
% cyc_fill_0 = cyc_raw;
% cyc_fill_0(cyc_fill_0 == 0) = cyc_raw_min;
% 
% cyc_log = log(cyc_fill_0);
% 
% data_suppress(:,1) = cyc_log;

for i = 1:4
    y_tmp = data_suppress(:, i);
    y_min = min(y_tmp(y_tmp > 0));
    y_fill_0 = y_tmp;
    y_fill_0(y_fill_0 == 0) = y_min;
    y_log = log(y_fill_0);
    data_suppress(:, i) = y_log;
end
...
for i = 1:num_component
    data_fit(:,i) = interp1(t, data_suppress(:, i), t_fit, 'spline');
end
