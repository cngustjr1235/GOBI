...
dimension = 4;   % dimension of the framework

%% all the pairs of 4D regulation (cause1, cause2, cause3, cause4, target)
...
parfor i = 1:num_data
    ...
    for j = 1:length(component_list_dim4(:,1))
        st1 = component_list_dim4(j,1); % index for cause1
        st2 = component_list_dim4(j,2); % index for cause2
        st3 = component_list_dim4(j,3); % index for cause3
        st4 = component_list_dim4(j,4); % index for cause4
        ed = component_list_dim4(j,5);  % index for target
        ...
        elseif type_self == 1
            [score_list, t_1, t_2] = RDS_ps_dim4(y_target(:,st1), y_target(:,st2), y_target(:,st3), y_target(:,st4), y_target(:,ed), t_target, time_interval);
        else
            [score_list, t_1, t_2] = RDS_dim4(y_target(:,st1), y_target(:,st2), y_target(:,st3), y_target(:,st4), y_target(:,ed), t_target, time_interval);
        end
...
filename = 'RDS_dim4';
save(filename, 'S_total_list', 'L_total_list','component_list_dim4', 'num_component', 'num_pair','num_type','num_data')

delete(gcp('nocreate'))

% plot_RDS_heatmap(dimension, component_list_dim2, S_total_list, labels);
