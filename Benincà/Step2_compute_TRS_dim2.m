...
y_var = [];
for i = 1:length(TRS_total)
    y_label = [];
    for j = 1:dimension+1
        idx = component_list_dim2(i, j);
        if idx == 1
            y_label = [y_label, ' Cyclopoids'];
        elseif idx == 2
            y_label = [y_label, ' Rotifers'];
        elseif idx == 3
            y_label = [y_label, ' Protozoa'];
        elseif idx == 4
            y_label = [y_label, ' Pico'];
        end
    end
    y_var = [y_var, convertCharsToStrings(y_label)];
end
