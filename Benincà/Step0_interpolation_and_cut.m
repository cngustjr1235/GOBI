...
y_raw = horzcat(data_ori.Cyclopoids, data_ori.Rotifers, data_ori.Picophytoplankton);
y_raw(isnan(y_raw)) = 0;
y = nthroot(y_raw, 4);
...
window_size_ori = 120;     % For oscillatory data, 1 period is recommended
...
% yticks([])
% legend('Rotifers', 'Picophytoplankton')

%% interpolate data
t_fit = linspace(t(1),t(end),length(t)/time_interval+1).';
